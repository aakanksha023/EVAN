...
The public sector of Vancouver is challenged with balancing tight budgetary constraints with an increasing demand for social services. With this project, our goal is to develop an understanding of the evolution of Vancouver’s business landscape to help mitigate this challenge and provide insight into the efficient allocation of city resources. To achieve the project goal, we will (1) develop a model to predict whether a business will renew its licence and (2) interpret the model output to provide a geospatial summary of the evolution of Vancouver’s business landscape. The baseline logistic regression model will be developed using features from a synthesis of business licence and census datasets. The final predictive model and geospatial visualization will be created with python and hosted on Heroku.
...
After the data is processed, we will use logistic regression to build a base line model. We choose logistic regression because it can produce a probability distribution of the predicted result, affording greater insight to the clients. Once we have developed the baseline model, we will implement regularization techniques to mitigate overfitting and model complexity. We may also draw on techniques from survival analysis to examine business renewal over extended periods. Parallel to the development of predictive models, we will also create a visualization to display geospatial information using Python and deploy the results.

The datasets pose some possible difficulties for us. First, the temporal and geospatial correlation between the variables must be identified and addressed. In addition, some variables may serve as proxies for underlying factors not included in the model. For example, a significant indicator may be the number of employees in the license dataset. Capital spent, however, may in fact be the hidden factor that influences both the number of employees and the likelihood of success.
...
```{r timelineimage, fig.cap="Timeline for whole project(red line is current project stage)", out.width="500px", out.height="300px", echo=FALSE}
